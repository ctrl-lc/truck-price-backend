...
projectdir = 'C:\\Users\\a.leshchenko\\OneDrive\\Документы\\Контрол\\Stocks'

# сортируем и кладем в full_stock1.csv

log ('Reading full_stock.csv and sorting by benefit')
df = pd.read_csv(projectdir + '\\data\\full_stock.csv', sep = ';')
df.sort_values(by=[u'Выгода'], ascending=False, inplace=True)
df.to_csv (projectdir + '\\data\\full_stock1.csv', index=False)
...
with timer('Loading stocks to Google Sheets'):
    ...
    credentials = ServiceAccountCredentials.from_json_keyfile_name(projectdir + '\\robotic-rampart-255014-e2f22bfae60e.json', scope)

    gc = gspread.authorize(credentials)

    content = codecs.open(projectdir + '\\data\\full_stock1.csv', 'r', 'utf-8').read()
